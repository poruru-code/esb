...
class TestUpCommand:
    ...
    def setup_env(self):
        ...
        with patch("tools.python_cli.commands.up.cli_config.TEMPLATE_YAML", "dummy_template.yaml"):
            yield
    ...
    def test_run_generates_certificates(
        self, mock_enforce, mock_cert, mock_prov, mock_sub, mock_args
    ):
        ...
        # Alternatively, we patch the source: tools.python_cli.core.cert.generate_ssl_certificate
    ...
    @patch("tools.python_cli.commands.up.subprocess.check_call")
    @patch("tools.python_cli.commands.up.provisioner.main")
    @patch("tools.python_cli.commands.up.ensure_certs")  # Expecting import in up.py
    @patch(
        "tools.python_cli.commands.up.cli_config.get_port_mapping",
        return_value={"ESB_PORT_GATEWAY_HTTPS": "443"},
    )
    @patch("requests.get")
    @patch("tools.python_cli.commands.up.context.enforce_env_arg")
