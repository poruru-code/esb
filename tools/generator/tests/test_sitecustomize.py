...
spec = importlib.util.spec_from_file_location(
    "sitecustomize", "cli/internal/generator/assets/site-packages/sitecustomize.py"
)
...
class TestSiteCustomize(unittest.TestCase):
    ...
    def test_log_shipping_success(self, mock_urlopen):
        ...
        hook = sitecustomize.VictoriaLogsStdoutHook(
            mock_stdout, container_name="test-container", vl_url="http://localhost:9428"
        )
        ...
        self.assertEqual(
            req.full_url,
            "http://localhost:9428/insert/jsonline?_stream_fields=container_name%2Cjob&_msg_field=message&_time_field=_time&container_name=test-container&job=lambda",
        )
