...
3. PR提出時は上記ドラフトをそのまま利用し、検証ログ（`runtime-safe` ESB 0 / unit / `--parallel` E2E）を添付する。

最終アクション（PR 1本化）:
1. `feat/branding-neutral-keys-phase2` を push する。
2. `docs/branding-phase2-pr-body.md` を本文として PR を作成する。
