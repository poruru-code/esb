# Where: tools/python_cli/tests/test_main.py
...
from tools.python_cli.main import main


@pytest.fixture(autouse=True)
def mock_template(monkeypatch):
    """Mock TEMPLATE_YAML to avoid early exit in main()."""
    monkeypatch.setattr("tools.python_cli.config.TEMPLATE_YAML", "fake-template.yaml")
    # Also skip context validation in main tests as they are dispatch tests
    monkeypatch.setattr(
        "tools.python_cli.core.context._validate_environment_initialized",
        lambda *args, **kwargs: None,
    )
    monkeypatch.setattr(
        "tools.python_cli.core.context._validate_environment_exists",
        lambda *args, **kwargs: None,
    )
    monkeypatch.setattr(
        "tools.python_cli.core.context._prompt_environment_selection",
        lambda: "default",
    )
    yield
...
@patch("tools.python_cli.commands.init.run")
@patch("tools.python_cli.config.set_template_yaml")
...
@patch("tools.python_cli.commands.logs.run")
...
@patch("tools.python_cli.commands.node.run")
