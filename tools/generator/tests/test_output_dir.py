...
def test_output_dir_relative_to_template(tmp_path, sample_template, sample_function_code):
    ...
    # Execute
    generator_main.generate_files(
        config=config, project_root=template_dir, dry_run=False, verbose=False
    )
...
def test_output_dir_absolute_path(tmp_path, sample_template, sample_function_code):
    ...
    config = {
        "app": {"name": "", "tag": "latest"},
        "paths": {"sam_template": str(template_file), "output_dir": str(output_dir) + "/"},
    }
...
def test_functions_yml_routing_yml_auto_derived(tmp_path, sample_template, sample_function_code):
    ...
    config = {
        "app": {"name": "", "tag": "latest"},
        "paths": {"sam_template": "template.yaml", "output_dir": "custom_output/"},
    }
