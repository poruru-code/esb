...
| `<BRAND>_VERSION` | なし | ビルド時の真のバージョン（必須）。 |
| `<BRAND>_TAG` | `<BRAND>_VERSION` | イメージタグ。原則 `<BRAND>_VERSION` と一致。 |
...
| `CONTAINER_REGISTRY` | (任意)                    | **Distribution**: Containerd モードでイメージをプルするレジストリのアドレス。HTTPS が必須（Insecure は非サポート）。 |
