...
- `artifactctl deploy` は artifact 内 function image の local registry alias（例: `127.0.0.1:5010`, `registry:5010`）を deploy 実行時の `CONTAINER_REGISTRY` に正規化して扱います。
...
- `artifactctl deploy` は最終 `CONFIG_DIR/functions.yml` の function image も実行時 registry へ正規化します。
