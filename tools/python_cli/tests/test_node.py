# Where: tools/python_cli/tests/test_node.py
...
def test_run_add_installs_key_and_saves_node(tmp_path):
    ...
    with (
        patch.object(node_cmd, "_read_payload", return_value=raw_payload),
        patch.object(node_cmd, "_ensure_local_keypair", return_value=(key_path, pub_path)),
        patch.object(node_cmd, "_install_public_key", return_value=True) as mock_install,
        patch.object(node_cmd, "_ssh_key_auth_ok", side_effect=[False, True]),
        patch.object(node_cmd, "_load_nodes", return_value={"version": 1, "nodes": []}),
        patch.object(node_cmd, "_save_nodes", side_effect=_save) as mock_save,
        patch.object(node_cmd, "_update_known_hosts") as mock_known,
    ):
...
def test_run_add_reinstalls_key_when_auth_fails(tmp_path):
    ...
    with (
        ...
        patch.object(node_cmd, "_ssh_key_auth_ok", side_effect=[False, True]),
        patch.object(node_cmd, "_load_nodes", return_value={"version": 1, "nodes": []}),
        patch.object(node_cmd, "_save_nodes", side_effect=_save),
        patch.object(node_cmd, "_update_known_hosts"),
    ):
        node_cmd._run_add(args)
...
def test_doctor_via_ssh_ok():
    ...
    with (
        patch.object(node_cmd, "_ssh_options", return_value=[]),
        patch("tools.python_cli.commands.node.subprocess.run", return_value=result),
    ):
...
def test_doctor_via_ssh_uses_sudo_when_nopasswd():
    ...
    with (
        patch.object(node_cmd, "_ssh_options", return_value=[]),
        patch("tools.python_cli.commands.node.subprocess.run", return_value=result) as mock_run,
    ):
...
def test_run_doctor_strict_exits_on_failure():
    ...
    with (
        patch.object(node_cmd, "_select_nodes", return_value=nodes),
        patch.object(node_cmd, "_doctor_via_ssh", return_value={"ok": False, "error": "fail"}),
...
def test_run_doctor_require_up_exits_when_down():
    ...
    with (
        patch.object(node_cmd, "_select_nodes", return_value=nodes),
        patch.object(node_cmd, "_doctor_via_ssh", return_value=payload),
        patch.object(node_cmd, "_render_doctor"),
    ):
        with pytest.raises(SystemExit) as exc:
            node_cmd._run_doctor(args)
...
def test_run_provision_sudo_nopasswd_no_prompt(tmp_path):
    ...
    with (
        patch.object(node_cmd, "_ensure_pyinfra"),
        patch.object(node_cmd, "_select_nodes", return_value=nodes),
        patch.object(node_cmd, "_ensure_wireguard_configs"),
        patch.object(node_cmd, "_prompt_secret", side_effect=AssertionError("prompted")),
        patch.object(node_cmd, "_run_pyinfra_deploy", side_effect=_capture),
...
def test_run_provision_prompts_for_sudo(tmp_path):
    ...
    with (
        ...
        patch.object(node_cmd, "_select_nodes", return_value=nodes),
        patch.object(node_cmd, "_ensure_wireguard_configs"),
        patch.object(node_cmd, "_prompt_secret", return_value="sudopass") as mock_prompt,
        patch.object(node_cmd, "_run_pyinfra_deploy", side_effect=_capture),
        patch.object(node_cmd, "_default_identity_file", return_value=None),
    ):
        node_cmd._run_provision(args)
...
def test_run_provision_passes_firecracker_settings(tmp_path):
    ...
    with (
        ...
        patch.object(node_cmd, "_select_nodes", return_value=nodes),
        patch.object(node_cmd, "_ensure_wireguard_configs"),
        patch.object(node_cmd, "_prompt_secret", return_value="sudopass"),
        patch.object(node_cmd, "_run_pyinfra_deploy", side_effect=_capture),
        patch.object(node_cmd, "_default_identity_file", return_value=None),
    ):
        node_cmd._run_provision(args)
...
def test_run_provision_passes_firecracker_assets_and_devmapper(tmp_path):
    ...
    assert (
        data["esb_firecracker_rootfs_path"] == "/var/lib/firecracker-containerd/runtime/rootfs.img"
    )
...
def test_run_up_requires_firecracker_mode():
    args = _args()
    with patch(
        "tools.python_cli.commands.node.runtime_mode.get_mode",
...
def test_run_up_starts_remote_compose(tmp_path, monkeypatch):
    ...
    with (
        ...
        patch("tools.python_cli.commands.node._select_nodes", return_value=nodes),
        patch(
            "tools.python_cli.commands.node.cli_compose.resolve_compose_files",
            return_value=[compose_path],
        ),
        patch("tools.python_cli.commands.node._upload_compose_files", return_value=[remote_path]),
        patch("tools.python_cli.commands.node._upload_support_files"),
        patch("tools.python_cli.commands.node._run_remote_command") as mock_run,
    ):
        node_cmd._run_up(args)
...
def test_run_up_uploads_support_files(tmp_path, monkeypatch):
    ...
    with (
        ...
        patch("tools.python_cli.commands.node._upload_compose_files", return_value=[remote_path]),
        patch("tools.python_cli.commands.node._upload_support_files") as mock_support,
        patch("tools.python_cli.commands.node._run_remote_command"),
    ):
        node_cmd._run_up(args)
...
def test_run_remote_command_quotes_command():
    ...
    with (
        patch.object(node_cmd, "_ssh_options", return_value=[]),
        patch("tools.python_cli.commands.node.subprocess.run") as mock_run,
    ):
