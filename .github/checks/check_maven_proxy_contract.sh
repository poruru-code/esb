...
if (( failures > 0 )); then
  ...
  echo "[maven-proxy-contract] If Maven is intentionally introduced in compose builds, integrate esb-ctl maven shim ownership (tools/cli/assets/mavenshim) and update this guard." >&2
  exit 1
fi
