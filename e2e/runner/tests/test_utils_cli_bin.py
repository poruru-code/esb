...
# Why: Keep CLI_BIN canonical key behavior stable with ESB_* fallback.
...
def test_build_esb_cmd_prefers_cli_bin() -> None:
    env = {
        ...
        constants.ENV_ESB_CLI: "/tmp/esb-cli",
        "ESB_BIN": "/tmp/esb-bin",
    }
...
def test_build_esb_cmd_falls_back_to_legacy_keys() -> None:
    env = {
        constants.ENV_ESB_CLI: "/tmp/esb-cli",
        "ESB_BIN": "/tmp/esb-bin",
    }
    cmd = build_esb_cmd(["status"], None, env=env)
    assert cmd[0] == "/tmp/esb-cli"
