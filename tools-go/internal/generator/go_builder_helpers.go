// Where: tools-go/internal/generator/go_builder_helpers.go
...
func stageConfigFiles(outputDir string, repoRoot string, env string) error {
...
func buildBaseImage(
	...
	if err := buildDockerImage(ctx, runner, runtimeDir, "Dockerfile.base", imageTag); err != nil {
		return err
	}
	if registry != "" {
...
func buildFunctionImages(
	...
	for _, fn := range functions {
		...
		if err := buildDockerImage(ctx, runner, outputDir, dockerfileRel, imageTag); err != nil {
			return err
		}
		if registry != "" {
...
func buildServiceImages(
	...
	for name, dir := range services {
		...
		if err := buildDockerImage(ctx, runner, dir, "Dockerfile", imageTag); err != nil {
			return err
		}
		if registry != "" {
...
func writeFunctionDockerignore(contextDir string, functionDir string) error {
