...
class TestBuildUpIntegration:
    ...
    @patch("tools.cli.commands.up.build.run")
    ...
    @patch("tools.cli.commands.up.proxy.prepare_env")
    @patch("tools.cli.commands.up.context.enforce_env_arg")
    def test_up_calls_build_when_flag_present(self, mock_enforce, mock_proxy_env, mock_cert, mock_prov, mock_sub, mock_build_run, mock_args):
        ...
        with patch("tools.cli.core.port_discovery.discover_ports", return_value={}), \
             patch("tools.cli.core.port_discovery.save_ports", return_value="ports.json"), \
             patch("tools.cli.core.port_discovery.apply_ports_to_env"), \
    ...
    @patch("tools.cli.commands.build.shutil")
    @patch("tools.cli.commands.build.cli_config.get_build_output_dir")
    @patch("tools.cli.commands.build.cli_config.get_env_name", return_value="demo")
    def test_build_always_stages_config(self, mock_get_env, mock_get_out, mock_shutil, mock_ensure_reg, mock_gen_files, mock_gen_load, mock_args, tmp_path):
        """Verify that 'esb build' always triggers staging logic"""
        # Set up output directory and mock it
        mock_out_dir = tmp_path / "out"
        from tools.cli import config as cli_lookup
        ...
        with patch.object(cli_lookup, "TEMPLATE_YAML", mock_template), \
             patch.object(cli_lookup, "E2E_DIR", tmp_path), \
             patch("tools.cli.commands.build.cli_config.TEMPLATE_YAML", mock_template), \
             patch("tools.cli.commands.build.cli_config.E2E_DIR", tmp_path):
            build.run(mock_args)
