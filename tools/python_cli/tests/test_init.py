...
from tools.python_cli.commands import init
...
def test_init_aborts_on_cancelled_input(tmp_path):
    ...
    with (
        patch("tools.python_cli.commands.init.cli_config") as mock_config,
        patch("tools.python_cli.commands.init.questionary") as mock_questionary,
    ):
...
def test_init_uses_cli_config_template(mock_template_yaml, tmp_path):
    ...
    with (
        patch("tools.python_cli.commands.init.cli_config") as mock_config,
        patch("tools.python_cli.commands.init.questionary.text") as mock_text,
        patch("tools.python_cli.commands.init.questionary.path") as mock_path,
        patch("tools.python_cli.commands.init.questionary.confirm") as mock_confirm,
    ):
...
def test_init_respects_overwrite_cancel(mock_template_yaml, tmp_path):
    ...
    with (
        ...
        patch("tools.python_cli.commands.init.questionary.text") as mock_text,
        patch("tools.python_cli.commands.init.questionary.path") as mock_path,
        patch("tools.python_cli.commands.init.questionary.select") as mock_select,
    ):
        mock_config.TEMPLATE_YAML = template_file
