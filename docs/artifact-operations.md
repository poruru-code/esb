...
- `esb-ctl deploy` may execute image build/pull when needed, but must not use artifact-time `runtime-base/**` as base source.
...
- `esb-ctl deploy` must ensure lambda base availability in the target registry even when no function image build targets exist.
...
Notes:
- `esb-ctl deploy` runs payload validation and artifact apply.
- `esb-ctl deploy` does not treat `runtime-base/**` as contract input.
- `esb-ctl deploy` may run image build/pull, but lambda base selection must follow deploy-time rules only.
- `esb-ctl deploy` ensures/pushes lambda base required by deploy-time function builds; when function build targets are absent, it ensures default `esb-lambda-base:latest`.
...
- `esb-ctl deploy` normalizes deploy-built function image refs from artifact-time local registry aliases (e.g. `127.0.0.1:5010`, `registry:5010`) to the current runtime registry (`CONTAINER_REGISTRY`) before build/push and output generation.
...
- deploy phases require `esb-ctl` on PATH (or `CTL_BIN` override)
