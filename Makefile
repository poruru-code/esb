# Where: Makefile
# What: Container image validation helpers (CST + Dockle).
# Why: Provide consistent local/CI entry points for image hygiene checks.
...
DOCKLE_TMP_VOLUME ?= esb-dockle-tmp
...
PYTHON_BASE_IMAGE ?= esb-python-base:latest
GATEWAY_IMAGE ?= gateway-api:latest
GATEWAY_FC_IMAGE ?= gateway-api-fc:latest
AGENT_IMAGE ?= esb-agent:latest
RUNTIME_NODE_IMAGE ?= esb-runtime-node:latest
RUNTIME_NODE_FC_IMAGE ?= esb-runtime-node-firecracker:latest
