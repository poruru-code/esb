...
from tools.python_cli.commands.reset import run as run_reset


# Common patches
@patch("builtins.input")
@patch("tools.python_cli.commands.reset.build.run")
@patch("tools.python_cli.commands.reset.down.run")
@patch("tools.python_cli.commands.reset.up.run")
@patch("tools.python_cli.commands.reset.context.enforce_env_arg")
def test_reset_command_cancel(mock_enforce, mock_up, mock_down, mock_build, mock_input):
    ...
    with patch("tools.python_cli.config.TEMPLATE_YAML", Path("dummy.yaml")):
        run_reset(args)
