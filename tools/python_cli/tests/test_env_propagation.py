...
def test_up_uses_env_argument_over_default(mock_env_vars):
    ...
    from tools.python_cli.commands import up
    ...
    with (
        ...
        patch("tools.python_cli.config.setup_environment"),
        patch("tools.python_cli.compose.build_compose_command"),
        patch("tools.python_cli.compose.resolve_compose_files", return_value=[]),
        patch("tools.python_cli.core.port_discovery.discover_ports", return_value={}),
        patch("tools.python_cli.core.port_discovery.save_ports"),
        patch("tools.python_cli.core.port_discovery.log_ports"),
        patch("tools.python_cli.core.context._validate_environment_initialized"),
        patch("tools.python_cli.core.context._validate_environment_exists"),
        patch("tools.python_cli.config.TEMPLATE_YAML", "fake.yaml"),
...
def test_reset_propagates_env_to_subcommands(mock_env_vars):
    ...
    # Import inside test
    from tools.python_cli.commands import reset

    args = Args(env=TEST_ENV)

    with (
        patch("tools.python_cli.commands.down.run"),
        patch("tools.python_cli.commands.build.run"),
        patch("tools.python_cli.commands.up.run"),
        patch("tools.python_cli.config.setup_environment"),
    ):
...
def test_main_arg_parsing_sets_env_and_calls_setup(mock_env_vars):
    ...
    from tools.python_cli import main
    ...
    with (
        ...
        patch("tools.python_cli.commands.up.run"),
        patch("tools.python_cli.config.setup_environment") as mock_setup,
        patch("tools.python_cli.config.TEMPLATE_YAML", "fake.yaml"),
    ):
        try:
            main.main()
...
def test_enforce_env_arg_sets_mode_from_generator(mock_env_vars, tmp_path):
    ...
    from tools.python_cli.core import context

    (tmp_path / "generator.yml").write_text("environments:\n  test_env: containerd\n")
    args = Args(env="test_env")

    with (
        patch.object(cli_config, "E2E_DIR", tmp_path),
        patch("tools.python_cli.config.setup_environment"),
    ):
