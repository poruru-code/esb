...
- `esb-ctl deploy`（およびそれを呼び出す外部 CLI ラッパー）は staging から実行中 compose stack の runtime-config mount を解決して同期します。
- 旧運用（runtime-config パス指定、`esb-ctl deploy --out`）はサポート対象外です。
