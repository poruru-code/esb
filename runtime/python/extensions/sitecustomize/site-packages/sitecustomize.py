...
# --- Globals & Originals ---
_ORIGINAL_CLIENT_ATTR = "_sitecustomize_original_client"
if getattr(boto3, _ORIGINAL_CLIENT_ATTR, None) is None:
    setattr(boto3, _ORIGINAL_CLIENT_ATTR, boto3.client)
_original_boto3_client = cast(
    Callable[..., Any],
...
def _is_stream_hooked(stream):
    return bool(getattr(stream, _STREAM_HOOK_FLAG, False))


def _is_victorialogs_hook_active():
    return _is_stream_hooked(sys.stdout) or _is_stream_hooked(sys.stderr)


def _should_direct_send(vl_url):
    return bool(vl_url) and not _is_victorialogs_hook_active()
...
class DynamicStdoutStreamHandler(logging.StreamHandler):
    """Write to the current sys.stdout so invocation-time hooks are respected."""
    ...
    def handle_put_log_events(cls, client, **kwargs):
        try:
            ...
            for event in log_events:
                ...
                # Direct send is fallback-only when stdout/stderr hook is unavailable.
                if _should_direct_send(vl_url):
...
class VictoriaLogsStdoutHook:
    ...
    def __init__(self, original_stream, container_name, vl_url):
        ...
        self._vl_url = vl_url
        setattr(self, _STREAM_HOOK_FLAG, True)
...
def _patch_awslambdaric():
    ...
    if getattr(original_handle_event_request, _AWSLAMBDA_PATCH_FLAG, False):
        _log_json("awslambdaric patch already applied.", level="DEBUG")
        return
    ...
    setattr(patched_handle_event_request, _AWSLAMBDA_PATCH_FLAG, True)
...
def _patch_logging():
    ...
    if getattr(root, _LOGGING_PATCH_FLAG, False):
    ...
    root.setLevel(os.environ.get("LOG_LEVEL", "INFO").upper())
    setattr(root, _LOGGING_PATCH_FLAG, True)
    _log_json("Patched standard logging to output JSON.")


# --- Apply Patches ---
if not getattr(boto3.client, _BOTO3_CLIENT_PATCH_FLAG, False):
    setattr(_patched_boto3_client, _BOTO3_CLIENT_PATCH_FLAG, True)
