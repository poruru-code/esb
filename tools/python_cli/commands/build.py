...
from tools.python_cli import build_service_images, runtime_mode
from tools.python_cli import compose as cli_compose
from tools.python_cli import config as cli_config
from tools.python_cli.core import logging, proxy
...
def _write_function_dockerignore(
    context_dir: Path, function_dir: Path, verbose: bool = False
) -> None:
    """Write a function-scoped .dockerignore to reduce build context size."""
    try:
        function_rel = function_dir.relative_to(context_dir)
    except ValueError:
        if verbose:
            logging.warning(
                f"Skipping .dockerignore generation (function outside context): {function_dir}"
            )
        return

    if not function_rel.parts or function_rel.parts[0] != "functions":
        if verbose:
            logging.info(
                f"Skipping .dockerignore generation (unexpected function path): {function_rel}"
            )
        return

    lines = [
        "# Auto-generated by esb build.",
        "# What: Limit Docker build context to the active function and shared layers.",
        "# Why: Reduce context upload size when using output_dir as build context.",
        "*",
        "!.dockerignore",
        "!functions/",
        f"!{function_rel.as_posix()}/",
        f"!{function_rel.as_posix()}/**",
        "!layers/",
        "!layers/**",
    ]
    content = "\n".join(lines) + "\n"
    (context_dir / ".dockerignore").write_text(content, encoding="utf-8")
...
def build_function_images(
    ...
    for func in functions:
        ...
        dockerfile_full_path = Path(dockerfile_path).resolve()
        ...
        context_dir = Path(context_path).resolve() if context_path else dockerfile_full_path.parent
        if not context_dir.exists():
            logging.warning(
                f"Build context not found for {function_name} at {context_dir.absolute()}"
            )
            continue

        try:
            dockerfile_relpath = dockerfile_full_path.relative_to(context_dir).as_posix()
        except ValueError:
            logging.warning(
                f"Dockerfile is outside build context for {function_name}: {dockerfile_full_path}"
            )
            continue

        _write_function_dockerignore(context_dir, dockerfile_full_path.parent, verbose=verbose)
...
from tools.python_cli.core import context  # noqa: E402
