...
class LocalLogHandler:
    ...
    def _estimate_container_name(log_group):
        # AWS_LAMBDA_FUNCTION_NAME を使用。未設定の場合は明示的に UNKNOWN を返す
...
def _patch_awslambdaric():
        ...
        def _send_to_victorialogs(self, message):
            ...
            try:
                ...
                try:
                    ...
                    # レベルの推測 (大文字小文字に関わらず、タグやキーワードを検索)
                ...
                # container_name と job を追加（既存の値を上書きしない）
    ...
    def patched_handle_event_request(
        ...
        **kwargs,
        ...
        # Hydration ログをキャプチャするために、ハイドレーションより先に行う
        ...
        try:
            ...
            if ctx:
                ...
                # 辞書またはオブジェクトから trace_id を抽出
                # Gatewayからは 'trace_id', AWS標準/boto3からは '_X_AMZN_TRACE_ID'
