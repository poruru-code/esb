...
def test_context_validation_checks_project_local_path(tmp_path):
    ...
    with patch("tools.python_cli.config.E2E_DIR", project_root):
...
def test_get_build_output_dir_respects_generator_yaml(tmp_path):
    ...
    with patch("tools.python_cli.config.E2E_DIR", dummy_e2e):
...
def test_set_template_yaml_expands_user():
    ...
    """
    import tools.python_cli.config as cli_config
    from tools.python_cli.config import set_template_yaml
